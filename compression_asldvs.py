...
from playgroundASLDVS import *
from pathlib import Path
from torch.utils.data import Subset

MODEL_SAVE_PATH = "./models/asldvs"
...
if __name__ == "__main__":
    ...
    data_path = "../datasets/ASL_DVS"

    # Load trained weight to model
    model = ASLDVS_4CONV_SNN(tau=2.0).to(DEV)
    model.load_state_dict(torch.load(model_path, weights_only=False, map_location=DEV)['model'])
    model.eval()
    print(model)
    ...
    with torch.no_grad():
        ...
        quant_data, _ = split_neuromorphic_data_subsets(train_dataset, [0.05, 0.95])
        #quant_data = train_dataset
        quant_test = test_dataset

        pre_prune_loss = 0.9653#validate_model(model, T, quant_test, 0.0, out_dir=MODEL_SAVE_PATH, batch=BATCH)
